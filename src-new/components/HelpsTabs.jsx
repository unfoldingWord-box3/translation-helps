...
const TABS = [
  ...
  { id: 'twl', label: 'Translation Word Links', component: TWLPanel }
];

export function HelpsTabs({ reference }) {
  const [activeTab, setActiveTab] = useState('tn');

  const ActiveComponent = TABS.find(tab => tab.id === activeTab)?.component || TranslationNotesPanel;
