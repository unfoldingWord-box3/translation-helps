...
describe("MainView", () => {
  it("renders reference selector, scripture panel, and helps tabs", () => {
    ...
    render(
      <ManifestsContext.Provider value={{ manifests: testManifests, isLoading: false }}>
        <ReferenceContext.Provider
          value={{ reference: testReference, setReference: () => {}, updateReference: () => {} }}
        >
          <ResourcesContext.Provider
            value={{ resources: testResources, loadResource: () => {}, isLoading: false }}
          >
            <MainView />
          </ResourcesContext.Provider>
        </ReferenceContext.Provider>
    ...
    // Note: TWL functionality is integrated into the TW tab, not a separate tab
  });
});
